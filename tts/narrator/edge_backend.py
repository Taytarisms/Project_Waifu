    ...
    async def narrate(
        ...
        *,
        ...
        try:
            ...
            Logger.warn(f"pydub conversion failed: {e}")
            self._safe_unlink(mp3_path)
            self._safe_unlink(wav_path)
            return False
        finally:
            # The mp3 is throwaway once we have the wav
            self._safe_unlink(mp3_path)

        ok = await asyncio.to_thread(
            self._play_wav_blocking, str(wav_path), device_index
        )
        self._safe_unlink(wav_path)
        return ok
