...
def main() -> int:
    ...
    for tier in tiers:
        ...
        print(f"[setup] Trying PyTorch runtime: {tier}")
        code = _run_pip([
            "install",
            "--upgrade",
            "torch",
        ...
        if code == 0:
            print(f"[setup] PyTorch runtime installed with {tier}.")
            return 0
        print(f"[setup] PyTorch runtime {tier} failed with exit code {code}.")

    print("[setup] ERROR: No compatible PyTorch runtime could be installed.")
    return 1
